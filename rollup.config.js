...
const getFiles = async (dir) => {
    ...
    dirents = dirents.filter((ent) => {
        if (
            ent.name === '3rdparty' ||
            ent.name === '3rdparty_customized' ||
            ent.name === '.eslintrc'
        ) {
            return false;
        }
...
const rollupConfig = files.map((file) => {
    ...
    if (namedExport === 'index') {
        namedExport = 'helpmateCss';
    }
    listOfImports.push(`import { ${namedExport} } from 'helpmate-css/dist/${source}.js';`);
    listOfRequires.push(`const { ${namedExport} } = require('helpmate-css/dist/${source}.cjs');`);
...
listOfImports.push(`import { helpmateCss } from 'helpmate-css';`);
listOfRequires.push(`const { helpmateCss } = require('helpmate-css');`);
